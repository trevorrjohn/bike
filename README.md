...
To run the `fix_dependency_order.bash` script after Android Studio re-writes your iml file. Go to Android Studio -> Preferences -> Compiler -> Command Line Options. And add `-D reorder=1` to trigger line 99 in build.gradle.
...
You must set the `output-test` directory in your iml file. This can be done manually and should be put under the `output` tag in the `component` tag and should look something like `<output-test url="file://$MODULE_DIR$/build/test-classes" />`.
